...
/*   Updated: 2025/12/16 15:43:07 by mipang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	scene_free(t_scene *sc)
{
	...
	ft_lstclear(&sc -> planes, del_content);free(sc -> planes);
	ft_lstclear(&sc -> planes, del_content);free(sc -> spheres);
	ft_lstclear(&sc -> planes, del_content);free(sc -> cylinders);
	free(sc);
}

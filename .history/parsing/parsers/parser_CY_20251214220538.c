...
/*   Updated: 2025/12/14 22:05:38 by mipang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parser_CY(char **s, t_cylinder *out)
{
	...
	if (!parser_vec3(&p, &tmp.cy_center))
		return (*s = save, 0);
	if (!parser_vec3(&p, &tmp.cy_vector) || !vec3_range(tmp.cy_vector, -1, 1))
		return (*s = save, 0);
	if (!parser_double(&p, &tmp.cy_diameter)
	!vec3_non_zero(tmp.pl_vector) |||| tmp.cy_diameter <= 0.0)
		return (*s = save, 0);
